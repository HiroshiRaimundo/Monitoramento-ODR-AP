...
interface JupyterNotebookTabProps {
  items: MonitoringItem[];
}

const JupyterNotebookTab: React.FC<JupyterNotebookTabProps> = ({ items }) => {
  if (items.length === 0) {
    return (
            ...
            <tr key={item.id} className="hover:bg-green-50/50">
              ...
              <td className="px-4 py-3 whitespace-nowrap text-sm text-right">
                <Button 
                  ...
                  onClick={() => downloadScript(item, 'jupyter', templates)}
                >
                  <Download size={14} />
                  <span>Notebook</span>
                </Button>
...
export default JupyterNotebookTab;
